...
namespace AndcultureCode.CSharp.Data.PostgreSQL.Repositories
{
    ...
    where T : Entity
    {
        ...
        public Repository(IContext context, IStringLocalizer localizer, ILogger<Repository<T>> logger)
        {
            Context = context;
        ...
        public virtual IResult<List<T>> BulkCreate(IEnumerable<T> entities, long? createdById = default(long?))
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"{errorKey}: {errorMessage}");
        ...
        public virtual IResult<List<T>> BulkCreateDistinct<TKey>(
            IEnumerable<T> entities, Func<T, TKey> property, long? createdById = default(long?)
        ) => BulkCreate(entities.GroupBy(property).Select(x => x.First()), createdById);
        ...
        public IResult<List<T>> CreateDistinct<TKey>(
            IEnumerable<T> items, Func<T, TKey> property, long? createdById = null
        ) => Create(items.GroupBy(property).Select(x => x.First()), createdById);
        ...
        {
            return this.BulkDelete(entities: items, deletedById: deletedById, soft: soft);
        }
        #endregion Delete
